...
@section('content')
    ...
    <main class="flex-1 overflow-auto p-4 sm:p-6 md:ml-64" x-data="archivesManager({
                                                            roles: {{ Js::from($roles->items()) }},
                                                            volunteers: {{ Js::from($volunteers->items()) }},
                                                            tasks: {{ Js::from($tasks->items()) }},
                                                            events: {{ Js::from($events->items()) }},
                                                            pagination: {
                                                                roles: {
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-600">Manage archived roles, volunteers, tasks, and events</p>
                </div>
            </div>
                ...
                <div class="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-indigo-900">{{ $roles->total() }}</p>
                        </div>
                ...
                <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-blue-900">{{ $volunteers->total() }}</p>
                        </div>
                ...
                <div class="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-yellow-900">{{ $tasks->total() }}</p>
                        </div>
                ...
                <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-purple-900">{{ $events->total() }}</p>
                        </div>
            ...
            <div x-show="tab === 'roles'" x-cloak>
                                ...
                                <tr>
                                    ...
                                    <td class="border border-gray-200 p-3">
                                        <div class="flex items-center gap-3">
                                            <div>
                                                <p class="font-medium text-gray-900">{{ $item['email'] }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="border border-gray-200 p-3">
                                        <span class="badge {{ $item['role'] === 'Admin' ? 'badge-success' : 'badge-info' }}">
                                            {{ $item['role'] }}
                                        </span>
                                    </td>
                                    <td class="border border-gray-200 p-3">{{ $item['ministry'] }}</td>
                                    <td class="border border-gray-200 p-3">{{ $item['archived_date'] }}</td>
                                    <td class="border border-gray-200 p-3">
                                        <span class="badge badge-warning">{{ $item['reason'] }}</span>
                                    </td>
                                    <td class="border border-gray-200 p-3">{{ $item['archived_by'] }}</td>
                                    <td class="border border-gray-200 p-3">
                                        <div class="flex items-center space-x-2">
                                            <button @click="restoreItem({{ json_encode($item) }})"
                                            ...
                                            <button @click="confirmDelete({{ json_encode($item) }})"
                ...
                {{-- Pagination --}}
                <div class="mt-4" id="roles-pagination">
                    {{ $roles->appends(['tab' => 'roles'])->links('vendor.pagination.tailwind') }}
                </div>
            </div>
            ...
            <div x-show="tab === 'volunteers'" x-cloak>
                <div class="overflow-x-auto">
                    <table class="w-full border-collapse table-hover">
                        ...
                        <tbody>
                            @foreach($volunteers->items() as $item)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="border border-gray-200 p-3 text-sm text-gray-700">{{ $item['email'] }}</td>
                                    <td class="border border-gray-200 p-3 text-sm text-gray-700">{{ $item['archived_date'] }}
                                    </td>
                                    <td class="border border-gray-200 p-3">
                                        <span class="badge badge-warning">{{ $item['reason'] }}</span>
                                    </td>
                                    <td class="border border-gray-200 p-3 text-sm text-gray-700">{{ $item['archived_by'] }}</td>
                                    <td class="border border-gray-200 p-3">
                                        <div class="flex items-center space-x-2">
                                            <button @click="restoreItem({{ json_encode($item) }})"
                                                class="inline-flex items-center px-3 py-2 text-sm border border-gray-300 bg-white text-gray-700 rounded-lg hover:bg-green-50 hover:border-green-300 hover:text-green-700 transition-all">
                ...
                {{-- Pagination --}}
                <div class="mt-4" id="volunteers-pagination">
                    {{ $volunteers->appends(['tab' => 'volunteers'])->links('vendor.pagination.tailwind') }}
                </div>
            </div>
            ...
            <div x-show="tab === 'tasks'" x-cloak>
                        ...
                        <tbody>
                            @foreach($tasks->items() as $item)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="border border-gray-200 p-3 text-sm text-gray-700">{{ $item['status'] }}</td>
                ...
                {{-- Pagination --}}
                <div class="mt-4" id="tasks-pagination">
                    {{ $tasks->appends(['tab' => 'tasks'])->links('vendor.pagination.tailwind') }}
                </div>
            </div>
            ...
            <div x-show="tab === 'events'" x-cloak>
                ...
                {{-- Pagination --}}
                <div class="mt-4" id="events-pagination">
                    {{ $events->appends(['tab' => 'events'])->links('vendor.pagination.tailwind') }}
                </div>
            </div>
            {{-- loading indicator --}}
    ...
    <script>
        function archivesManager(initialData) {
            return {
                ...
                init() {
                    // Show roles pagination by default, hide others
                    document.querySelectorAll('[id$="-pagination"]').forEach(el => {
                        el.style.display = 'none';
                    });
                    document.getElementById('roles-pagination').style.display = 'block';
                ...
                getTabCount(tabName) {
                    return this.pagination[tabName].total;
                },

                // Add method to handle pagination
                changePage(tab, page) {
                    this.loading = true;

                    fetch(`/archives/paginate?tab=${tab}&page=${page}`, {
                        headers: {
                        ...
                        .then(data => {
                            ...
                            this.pagination[tab] = data.pagination;

                            // Update pagination HTML
                            document.getElementById(`${tab}-pagination`).innerHTML = data.pagination.links;
                        ...
                        .catch(error => {
                            ...
                            this.loading = false;
                            this.showToastMessage('Error loading page', 'error');
                        });
                },
                changeTab(newTab) {
                    this.tab = newTab;
                    ...
                    // Show the appropriate pagination and hide others
                    document.querySelectorAll('[id$="-pagination"]').forEach(el => {
                        el.style.display = 'none';
                    });

                    // Show the current tab's pagination
                    const currentPagination = document.getElementById(`${newTab}-pagination`);
                ...
                toggleAll(event) {
                    ...
                    if (event.target.checked) {
                        checkboxes.forEach(checkbox => {
                            ...
                            if (checkbox.value && !this.selectedItems.includes(checkbox.value)) {
