    ...
    <div class="flex-1 overflow-auto py-6">
        ...
        <nav class="grid gap-1 px-4">
            ...
            <a href="{{ url('/settings') }}" class="group flex items-center gap-3 rounded-xl px-4 py-3 transition-all duration-200 {{ navActive('settings') }}">
                <div class="flex items-center justify-center w-8 h-8 rounded-lg transition-colors duration-200 {{ request()->is('settings') ? 'bg-white/20' : 'bg-slate-100 group-hover:bg-slate-200' }}">
                    <i class="bx bx-cog text-lg {{ request()->is('settings') ? 'text-white' : 'text-slate-600' }}"></i>
                </div>
                <div class="flex flex-col">
                    <span class="font-medium text-sm">Account Settings</span>
                    <span class="text-xs {{ request()->is('settings') ? 'text-white/70' : 'text-slate-500 group-hover:text-slate-600' }}">Profile & preferences</span>
                </div>
            </a>
...
<style>
    /* Ensure consistent background with dashboard */
    .settings-content {
        background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%);
        min-height: 100vh;
    }

    /* Custom scrollbar for settings sidebar */
