...
$events = [
    ...
    [
        ...
        'startTime'=>'6:30 PM','endTime'=>'8:30 PM','location'=>'Youth Room',
    ...
    [
        ...
        'startTime'=>'7:00 PM','endTime'=>'8:00 PM','location'=>'Chapel',
...
<div class="flex-1 flex flex-col overflow-auto p-6 space-y-6">
  ...
  <div class="flex justify-between items-center">
    ...
    <div class="flex items-center space-x-4">
      ...
      <button id="newEventBtn" class="flex items-center bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
      ...
      <div class="bg-white shadow rounded">
        ...
        <div class="p-4 max-h-72 overflow-auto space-y-2">
          @foreach($events as $ev)
            <div
              class="flex items-start cursor-pointer hover:bg-gray-50 rounded-md p-2"
...
<script>
  ...
  function renderMonthView(){
    ...
    for(let d=1; d<=daysInMonth; d++){
      ...
      html += `<div class="p-2 min-h-[80px] border rounded-md ${isToday?'bg-gray-200':''}" onclick="selectDay(${year},${month},${d})">`;
      ...
      cellEvents.forEach(ev=>{
        html += `<div class="${ev.color} text-white p-1 rounded text-xs mt-1 truncate cursor-pointer" onclick="event.stopPropagation();openViewEvent(${ev.id});">${ev.title}</div>`;
      });
      html += '</div>';
    }
    html += '</div>';
  ...
  function renderWeekView(){
    ...
    for(let i=0;i<12;i++){
      const hr = i+8, disp = hr>12?hr-12:hr, pm = hr>=12?'PM':'AM';
  ...
  function renderDayView(){
    ...
    const header = date.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'});
    let html = `<div class="border rounded-md"><div class="p-4 border-b font-medium text-center">${header}</div><div class="grid grid-cols-1">`;
    for(let i=0;i<12;i++){
      const hr = i+8, disp = hr>12?hr-12:hr, pm = hr>=12?'PM':'AM';
      html += `<div class="p-2 border-b flex"><div class="w-20 text-sm">${disp} ${pm}</div><div class="flex-1 min-h-[60px]"></div></div>`;
    }
    html += '</div></div>';
    calendarView.innerHTML = html;
  }
